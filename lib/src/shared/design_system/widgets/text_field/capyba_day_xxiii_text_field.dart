...
import 'package:capyba_day_xxiii/src/shared/design_system/themes/app_text_field_color.dart';
...
class CapybaDayXxiiiTextField extends StatefulWidget {
  final String hintText;
  final CapybaDayXxiiiTextFieldController? controller;
  ...
  @override
  _CapybaDayXxiiiTextFieldState createState() =>
      _CapybaDayXxiiiTextFieldState();
}

class _CapybaDayXxiiiTextFieldState extends State<CapybaDayXxiiiTextField> {
  late BehaviorSubject<bool> focusSubject;
  late FocusNode focusNode;
  late void Function() focusListener;
  late CapybaDayXxiiiTextFieldController controller;
  @override
  void initState() {
    if (widget.controller == null) {
      controller = CapybaDayXxiiiTextFieldController(null);
    } else {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<_TextFieldState>(
      initialData: _TextFieldState(
        const CapybaDayXxiiiTextFieldControllerState(),
        widget.autofocus,
      ),
      stream: Rx.combineLatest2<CapybaDayXxiiiTextFieldControllerState, bool,
          _TextFieldState>(
        controller.stream,
        focusSubject.stream,
  ...
  ThemeData generateTheme(
    ...
    CapybaDayXxiiiTextFieldControllerState? state,
  ) {
    ...
    return currentTheme.copyWith(
      inputDecorationTheme: currentTheme.inputDecorationTheme.copyWith(
        prefixStyle: CapybaDayXxiiiTextStyle.bodyText1.style.copyWith(
          color: widget.color.prefix,
        ),
        contentPadding: const EdgeInsets.symmetric(
          vertical: SpacingTokens.tera,
        ),
        hintStyle: CapybaDayXxiiiTextStyle.headline5.style.copyWith(
          color: widget.color.hintText,
        ),
        errorStyle: CapybaDayXxiiiTextStyle.caption.style.copyWith(
  ...
  TextStyle style(bool hasFocus) {
    if (hasFocus == true) {
      return CapybaDayXxiiiTextStyle.headline5.style.copyWith(
...
class _TextFieldState {
  final bool hasFocus;
  final CapybaDayXxiiiTextFieldControllerState state;
...
class _Prefix extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        text,
        style: CapybaDayXxiiiTextStyle.bodyText2.style.apply(color: textColor),
      ),
    );
  }
}
